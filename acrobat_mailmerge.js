...
AcrobatMailMerge.prototype.print = function(batchSize, startIndex, endIndex){
	...
	function replacer(match, p1){
		...
		// DEBUG
		console.println("Matched " + match + " and captured " + p1 + ". Merging " + data[i][p1]);
		return "<" + data[i][p1] + ">";
	};
	...
	for (var i = startIndex; i < endIndex + 1; i++){
		...
		for (var k=0; k < textBoxes.length; k++){
			...
			// DEBUG
			console.println("data[i]['test'] is " + data[i]["test"]);
			console.println("Textbox value BEFORE merge: " + textBoxes[k].value);
			
			// replace <<fieldnames>> with <values>
			textBoxes[k].value = textBoxes[k].value.replace( this.locals.targetFieldsRegex , replacer);

			// DEBUG
			console.println("Textbox value AFTER merge: " + textBoxes[k].value);
